...
begin
    ...
    $s \leftarrow \prod_{i=l}^{|K|-1} K_i$
    $p' \leftarrow \text{Sample}(p, s)$
    $k \leftarrow \text{K Medoids}(p', K_l)$ 
    $m \leftarrow \min_{q \in k}|q|$
    $c \leftarrow \emptyset$
    for $q \in k$ do
    begin
        $q'' \leftarrow \Set{ q' \subseteq q | m = |q'| }$
        $g \leftarrow$ centro de gravidade em $q$
        $q \leftarrow \argmin_{q' \in q''} \sum_{v \in q'}d(v, g)$
        $c \leftarrow c \cup \text{cover}(l+1, q)$
    end
